...
$arrcompany=pg_fetch_all_columns($qcompany, 1);
				...
				if (isset($_COOKIE['id']) and isset($_COOKIE['hash'])) 
		{
			...
			//cкважина не выбрана
			if ($num_schv==0) 
			 $res_scv = pg_query($dbconn, "select t1.name_mest,t1.koord,t1.number_scv,t1.status,t3.name,t3.email,v1.company,t1.radius,t4.koord
     		 from scvazh t1 inner join neft t2 on t1.name_mest=t2.name 
			inner join v_neft_prin_comp v1 on v1.id=t2.id inner join users t3 on t1.employee=t3.user_id inner join neft t4 on t1.name_mest=t4.name
            where v1.company LIKE '%{$user_company}%'");
			else  if ($num_schv!=0) 
				$res_scv = pg_query($dbconn, "select t1.name_mest,t1.koord,t1.number_scv,t1.status,t3.name,t3.email,v1.company,t1.radius,t4.koord from scvazh t1 inner join neft t2 on t1.name_mest=t2.name 
			inner join v_neft_prin_comp v1 on v1.id=t2.id inner join users t3 on t1.employee=t3.user_id inner join neft t4 on t1.name_mest=t4.name
			...
			$coord_mest=pg_fetch_all_columns($res_scv, 8);
...
if ($num_schv==0) 
	for ($i=0;$i<count($coord_scv);$i++)
			...
			echo "addscv({coordinates:["; echo $coord_scv[$i]; echo "],";
			   ...
			   echo "company:"; echo "'{$user_company}'"; echo ",";
			   echo "coord_mest:'"; echo $coord_mest[$i]; echo "'";
			   echo "});";    
			}
			...
			group3[qq].addTo(map);
			  group3[qq].on('click', function(e) {
				   ...
				   map.setView([e.latlng.lat, e.latlng.lng],30);
							 ...
							 '<br><hr><center><a href="javascript:info(numb_scv);" style="color: #00FFFF">Показать состояние скважины</a></center>'+
						...
						function addscv1(properties)
{
			...
			group3[qq].addTo(map);
				   ...
				   map.setView([e.latlng.lat, e.latlng.lng],30);
							 ...
							 '<br><b><font size=4>Компания: </b>  '+properties.company+''+'</font>'+
							 '<br><hr><center><a href="javascript:info(numb_scv);" style="color: #00FFFF">Показать состояние скважины</a></center>'+
						     '<br><center><input type="button" value="Скрыть информацию" onclick="removediv4(),monitor()"></center>';
                             document.body.appendChild(div4);	
			  });  
				  							if (typeof div4 !=="undefined" && div4!=="0"){ div4.parentNode.removeChild(div4);}			
							 div4 = document.createElement('div4');
