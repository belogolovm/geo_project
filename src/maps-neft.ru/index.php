...
$.ajax({
  type: 'POST',
  url: 'show2.php?action=sample2',
 data:  "nazv="+mest+"&num_schv="+numb_scv,
  success: function(data){
	$('.results').html(data);
  }
  });*/
...
window.setInterval(function(){
	//каждые 4 секунды если включен мониторинг (есть куки) запускать функцию monitor и datch_timer(если есть overlay3)	
 var monitoring_cookie = getCookie("monitoring");
 							var myCookie = getCookie("id");
